// Standalone bit-exact gate for the 2-stage pipelined fp32_sqrt_p2 module.
//
...
// cycle and outputs are checked against a 2-deep golden queue (LATENCY=2).
// Links ONLY fp32_sqrt_p2 (no taccel_top) — zero risk to the proven cosim.

#include "Vfp32_sqrt_p2.h"
#include "verilated.h"
...
static Vfp32_sqrt_p2* g_dut = nullptr;
...
int main(int argc, char** argv) {
  ...
  delete g_dut;
  std::printf("fp32_sqrt_p2: checked=%ld  mismatches=%d\n", g_checked, g_fail);
  if (g_fail == 0) { std::printf("PASS: fp32_sqrt_p2 bit-exact vs host float\n"); return 0; }
  std::fprintf(stderr, "FAIL: fp32_sqrt_p2 %d mismatches\n", g_fail);
  return 1;
}
