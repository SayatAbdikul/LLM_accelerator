// Standalone bit-exact gate for the 6-stage pipelined fp32_div_p6 module.
//
...
// cycle and outputs are checked against a golden queue (LATENCY=6; the
// in-flight deque auto-handles any latency).
// Links ONLY fp32_div_p6 (no taccel_top) — zero risk to the proven cosim.

#include "Vfp32_div_p6.h"
#include "verilated.h"
...
static Vfp32_div_p6* g_dut = nullptr;
...
int main(int argc, char** argv) {
  ...
  delete g_dut;
  std::printf("fp32_div_p6: checked=%ld  mismatches=%d\n", g_checked, g_fail);
  if (g_fail == 0) { std::printf("PASS: fp32_div_p6 bit-exact vs host float\n"); return 0; }
  std::fprintf(stderr, "FAIL: fp32_div_p6 %d mismatches\n", g_fail);
  return 1;
}
