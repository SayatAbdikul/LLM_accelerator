// Standalone bit-exact gate for the 3-stage pipelined fp32_sqrt_p3 module.
//
...
// cycle and outputs are checked against a golden queue (LATENCY=3, but the
// in-flight deque auto-handles any latency).
// Links ONLY fp32_sqrt_p3 (no taccel_top) — zero risk to the proven cosim.

#include "Vfp32_sqrt_p3.h"
#include "verilated.h"
...
static Vfp32_sqrt_p3* g_dut = nullptr;
...
int main(int argc, char** argv) {
  ...
  delete g_dut;
  std::printf("fp32_sqrt_p3: checked=%ld  mismatches=%d\n", g_checked, g_fail);
  if (g_fail == 0) { std::printf("PASS: fp32_sqrt_p3 bit-exact vs host float\n"); return 0; }
  std::fprintf(stderr, "FAIL: fp32_sqrt_p3 %d mismatches\n", g_fail);
  return 1;
}
