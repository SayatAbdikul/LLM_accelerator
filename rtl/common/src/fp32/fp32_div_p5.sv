// 5-stage pipelined IEEE-754 binary32 divide — bit-exact to the combinational
// `fp32_div` (and to fp32_div_p2/p3/p4, and the DPI golden `sfu_fp32_div`).
//
// WHY THIS EXISTS:
//   After fp32_div_p4 + fp32_sqrt_p4, the SFU post-PNR fmax floor is the
//   divider's STAGE 2/3 (the 8-iter middles of the 6/8/8/7 split, ~36.2 ns at
//   PNR, binding across two instances u_ln_var_norm + u_ln_norm — the entire
//   top-50 PNR endpoint tier is div_p4 rB_P/rC_P). A FIFTH stage cuts the
//   29-iteration restoring divide into ~6-iter pieces, dropping the divider
//   below the next floor cluster.
//
// EQUIVALENCE:
//   Identical arithmetic to fp32_div_p4 — same unpack, subnormal normalize,
...
//   change is WHERE the iteration chain is cut: 4 pipeline registers split it
//   five ways. The restoring divide is value-identical regardless of where it is
//   cut (each iteration's compare/subtract/shift is unchanged), so q_full /
//   remainder are bit-identical. test_fp32_div_p5.cpp (built against this module
//   with LATENCY=5) is the byte-exact gate.
//
...
//   SPLIT3-1..SPLIT4, stage 5 SPLIT4-1..0 plus round/pack. Divider iterations are
//   ~uniform cost (compare is always 25-bit), so the split is near-even; stage 1
//   gets fewer iters (carries unpack/normalize), stage 5 fewer (carries
//   round/pack). SPLIT1=24,SPLIT2=18,SPLIT3=12,SPLIT4=6 -> 5 / 6 / 6 / 6 / 6 iters.
//
// INTERFACE: fixed LATENCY=5. Present (a,b,valid_in) on cycle N; (y,valid_out)
//   are valid on cycle N+5. Fully pipelined: accepts a new pair every cycle.

`ifndef FP32_DIV_P5_SV
`define FP32_DIV_P5_SV
...
module fp32_div_p5 (
  ...
  // Stage3: SPLIT2-1..SPLIT3 ; Stage4: SPLIT3-1..SPLIT4 ; Stage5: SPLIT4-1..0.
  // SPLIT1=24,SPLIT2=18,SPLIT3=12,SPLIT4=6 -> 5 / 6 / 6 / 6 / 6 iters.
  localparam int SPLIT1 = 24;
  localparam int SPLIT2 = 18;
  localparam int SPLIT3 = 12;
  localparam int SPLIT4 = 6;
  ...
  // =====================================================================
  // STAGE 5 (combinational): division iters SPLIT4-1 .. 0, RNE round, pack.
  // =====================================================================
  logic [24:0] s5_P;
  logic [28:0] q_full;
  ...
  always_comb begin
    s5_P   = rD_P;
    q_full = rD_q;
    for (k5 = SPLIT4 - 1; k5 >= 0; k5 = k5 - 1) begin
    ...
    remainder = s5_P;            // final partial remainder = dividend mod sig_b
  end

  // --- extract mantissa + RNE on the regime ULP (verbatim from fp32_div_p4) ---
  ...
  always_comb begin
    if (rD_agb) begin
      ...
      exp_y_unb  = rD_expa - rD_expb;
    end else begin
      mant24_pre = q_full[27:4];
      rb         = q_full[3];
      st         = (|q_full[2:0]) | (|remainder);
      exp_y_unb  = rD_expa - rD_expb - 11'sd1;
    end
  ...
  // Subnormal/underflow path (verbatim from fp32_div_p4).
  ...
  always_comb begin
    k_lsb_s       = -{{2{rD_expa[9]}}, rD_expa} + {{2{rD_expb[9]}}, rD_expb} - 12'sd121;
  ...
  logic [31:0] y_comb;
  always_comb begin
    if (rD_anan || rD_bnan) begin
      y_comb = QNAN;
    end else if (rD_ainf && rD_binf) begin
      y_comb = QNAN;
    end else if (rD_azero && rD_bzero) begin
      y_comb = QNAN;
    end else if (rD_ainf) begin
      y_comb = {rD_sign, 8'd255, 23'd0};
    end else if (rD_binf) begin
      y_comb = {rD_sign, 8'd0,   23'd0};
    end else if (rD_bzero) begin
      y_comb = {rD_sign, 8'd255, 23'd0};
    end else if (rD_azero) begin
      y_comb = {rD_sign, 8'd0,   23'd0};
    end else if (exp_y_final_unb >= 11'sd128) begin
      y_comb = {rD_sign, 8'd255, 23'd0};
    end else if (exp_y_final_unb < -11'sd126) begin
      if (mant_sub_rnd[23])
        y_comb = {rD_sign, 8'd1, 23'd0};
      else
        y_comb = {rD_sign, 8'd0, mant_sub_rnd[22:0]};
    end else if (carry) begin
      y_comb = {rD_sign, exp_y_biased, 23'd0};
    end else begin
      y_comb = {rD_sign, exp_y_biased, mant24_rnd[22:0]};
    end
  end

  // ---- output register (stage-5 path ends at a register) ----
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      valid_out <= 1'b0;
    end else begin
