// 4-stage pipelined IEEE-754 binary32 divide — bit-exact to the combinational
// `fp32_div` (and to fp32_div_p2 / fp32_div_p3, and the DPI golden
// `sfu_fp32_div`).
//
// WHY THIS EXISTS:
//   After fp32_div_p3 (3-stage) + fp32_sqrt_p3, the SFU post-PNR fmax floor is
//   the divider's STAGE 2 (the 12-iter middle of the 8/12/9 split, ~53.8 ns at
//   PNR, binding at two instances u_ln_var_norm + u_sm_div). A 3-stage rebalance
//   bottoms out ~53 ns (stages 1/3 carry unpack/pack overhead), still above the
//   in-SFU sqrt path (~48.8 ns), so the divider would stay binding. A FOURTH
//   stage cuts the 29-iteration restoring divide into ~7-iter pieces (~39 ns
//   standalone synth), dropping the divider below the next floor cluster.
//
// EQUIVALENCE:
//   Identical arithmetic to fp32_div_p3 — same unpack, subnormal normalize,
...
//   change is WHERE the iteration chain is cut: 3 pipeline registers split it
//   four ways. The restoring divide is value-identical regardless of where it
//   is cut (each iteration's compare/subtract/shift is unchanged), so
//   q_full/remainder are bit-identical. test_fp32_div_p4.cpp (built against this
//   module with LATENCY=4) is the byte-exact gate.
//
...
//   SPLIT3-1..0 plus round/pack. Divider iterations are ~uniform cost (compare
//   is always 25-bit), so the split is near-even; stage 1 gets fewer iters
//   (carries unpack/normalize), stage 4 fewer (carries round/pack).
//   SPLIT1=23,SPLIT2=15,SPLIT3=7 -> 6 / 8 / 8 / 7 iters.
//
// INTERFACE: fixed LATENCY=4. Present (a,b,valid_in) on cycle N; (y,valid_out)
//   are valid on cycle N+4. Fully pipelined: accepts a new pair every cycle.

`ifndef FP32_DIV_P4_SV
`define FP32_DIV_P4_SV
...
module fp32_div_p4 (
  ...
  // Stage3: SPLIT2-1..SPLIT3 ; Stage4: SPLIT3-1..0.
  // SPLIT1=23,SPLIT2=15,SPLIT3=7 -> 6 / 8 / 8 / 7 iters.
  localparam int SPLIT1 = 23;
  localparam int SPLIT2 = 15;
  localparam int SPLIT3 = 7;
  ...
  // =====================================================================
  // STAGE 4 (combinational): division iters SPLIT3-1 .. 0, RNE round, pack.
  // =====================================================================
  logic [24:0] s4_P;
  logic [28:0] q_full;
  ...
  always_comb begin
    s4_P   = rC_P;
    q_full = rC_q;
    for (k4 = SPLIT3 - 1; k4 >= 0; k4 = k4 - 1) begin
    ...
    remainder = s4_P;            // final partial remainder = dividend mod sig_b
  end

  // --- extract mantissa + RNE on the regime ULP (verbatim from fp32_div_p3) ---
  ...
  always_comb begin
    if (rC_agb) begin
      ...
      exp_y_unb  = rC_expa - rC_expb;
    end else begin
      mant24_pre = q_full[27:4];
      rb         = q_full[3];
      st         = (|q_full[2:0]) | (|remainder);
      exp_y_unb  = rC_expa - rC_expb - 11'sd1;
    end
  ...
  // Subnormal/underflow path (verbatim from fp32_div_p3).
  ...
  always_comb begin
    k_lsb_s       = -{{2{rC_expa[9]}}, rC_expa} + {{2{rC_expb[9]}}, rC_expb} - 12'sd121;
  ...
  logic [31:0] y_comb;
  always_comb begin
    if (rC_anan || rC_bnan) begin
      y_comb = QNAN;
    end else if (rC_ainf && rC_binf) begin
      y_comb = QNAN;
    end else if (rC_azero && rC_bzero) begin
      y_comb = QNAN;
    end else if (rC_ainf) begin
      y_comb = {rC_sign, 8'd255, 23'd0};
    end else if (rC_binf) begin
      y_comb = {rC_sign, 8'd0,   23'd0};
    end else if (rC_bzero) begin
      y_comb = {rC_sign, 8'd255, 23'd0};
      ...
      if (mant_sub_rnd[23])
        y_comb = {rC_sign, 8'd1, 23'd0};
      else
        y_comb = {rC_sign, 8'd0, mant_sub_rnd[22:0]};
    end else if (carry) begin
      y_comb = {rC_sign, exp_y_biased, 23'd0};
    end else begin
      y_comb = {rC_sign, exp_y_biased, mant24_rnd[22:0]};
    end
  end
