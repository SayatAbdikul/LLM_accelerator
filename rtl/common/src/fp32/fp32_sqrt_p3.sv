// 3-stage pipelined IEEE-754 binary32 square root — bit-exact to the
// combinational `fp32_sqrt` (and thus to the DPI golden `sfu_fp32_sqrt`).
//
// WHY THIS EXISTS:
//   After fp32_div_p3, the post-PNR fmax floor became the sqrt STAGE-2 of
//   fp32_sqrt_p2 (the SFU LN denom path, u_ln_sqrt.y, ~57.8 ns at PNR) — the
//   8 widest digit-recurrence iters (7..0) plus RNE/pack, all in one stage.
//   This module adds a third pipeline cut so that wide tail is split across
//   two stages, dropping the per-stage delay toward the next floor cluster
//   (~45-50 ns: other div/sqrt stages, fp32 adds, gelu).
//
// EQUIVALENCE:
//   Identical arithmetic to fp32_sqrt / fp32_sqrt_p2 (unpack, subnormal
//   normalize, even-exponent radicand build, restoring radix-2 recurrence,
//   RNE rounding, non-finite contract, pack). Only the placement of pipeline
//   registers differs. test_fp32_sqrt_p3.cpp is the byte-exact streaming gate.
//
// SPLIT POINTS (balance by DELAY, not iter count — the recurrence cost is
//   NOT uniform: early iters have a narrow q [leading-zero-folded, cheap],
//   later iters a wide q -> full 52-bit compare/subtract, expensive):
...
//       2/3 see only zero blocks (no M_pad register needed past stage 1),
//       exactly the structure fp32_sqrt_p2 relied on at its single cut.
//     Stage 2: iters S1_LO-1 .. S2_LO   (zero blocks)
//     Stage 3: iters S2_LO-1 .. 0       (zero blocks) + RNE round + pack
//   The S1_LO / S2_LO constants below are tuned from per-stage STA; adjust
//   them (re-measure) if the binding stage shifts.
//
// INTERFACE: fixed LATENCY=3. Present (a,valid_in) on cycle N; (y,valid_out)
//   are valid on cycle N+3. Fully pipelined: accepts a new operand every cycle.

`ifndef FP32_SQRT_P3_SV
`define FP32_SQRT_P3_SV
...
module fp32_sqrt_p3 (
  ...
  localparam int S1_LO = 10;   // stage 1: iters 24 .. S1_LO
  localparam int S2_LO = 5;    // stage 2: iters S1_LO-1 .. S2_LO
  ...
  // No M_pad bits needed: stages 2/3 consume M_pad[2*S1_LO-1:0], all zero.
  logic              rA_valid;
  logic [51:0]       rA_r;
  logic [24:0]       rA_q;
  ...
  // =====================================================================
  // STAGE 3 (combinational): iters S2_LO-1 .. 0 (zero blocks), RNE, pack.
  // =====================================================================
  logic [51:0] s3_r;
  logic [24:0] q_final;
  integer i3;
  always_comb begin
    s3_r    = rB_r;
    q_final = rB_q;
    for (i3 = S2_LO-1; i3 >= 0; i3 = i3 - 1) begin
      logic [51:0] rn;
      logic [51:0] tr;
  ...
  always_comb begin
    carry        = mant24_rnd[24];
    exp_y_unb    = rB_Rexp + (carry ? 10'sd1 : 10'sd0);
  ...
  always_comb begin
    if (rB_anan) begin
      y_comb = QNAN;
    end else if (rB_negf) begin
      y_comb = QNAN;                                   // sqrt(neg) -> NaN
    end else if (rB_azero) begin
      y_comb = {rB_s, 8'd0, 23'd0};                    // ±0 -> ±0
    end else if (rB_ainf) begin
      y_comb = rB_s ? QNAN : {1'b0, 8'd255, 23'd0};    // -inf -> NaN, +inf -> +inf
