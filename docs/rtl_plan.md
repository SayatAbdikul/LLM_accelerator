...
The software ISA has 23 defined instructions. `software/docs/isa_spec.md` is
the source of truth for encodings, reserved fields, and execution semantics.
...
- RTL support for the current masked-attention ISA (`CONFIG_ATTN`,
  `MASKED_SOFTMAX`, `MASKED_SOFTMAX_ATTNV`) is being brought up against the
  golden model. The reserved-opcode range is `0x17-0x1F`.
...
- `SCALE_MUL`
- `VADD`
- Full regression coverage for masked-attention program fragments generated by
...
|---|---|---|---|
| 23-op decode table | No | Yes | In progress |
...
- Update `taccel_pkg`, decoder, and control logic to the current 23-op ISA
...
1. Align RTL decode/control with the current 23-op software ISA.
